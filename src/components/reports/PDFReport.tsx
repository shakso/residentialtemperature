import { jsPDF } from 'jspdf';
import 'jspdf-autotable';
import { format } from 'date-fns';

// Check if we're in Node.js environment
const isNode = typeof window === 'undefined';

interface TemperatureStats {
  min_temp: number;
  max_temp: number;
  avg_temp: number;
  min_temp_sensor_name: string;
  max_temp_sensor_name: string;
  min_temp_time: string;
  max_temp_time: string;
}

interface SensorTemperatureStats {
  sensor_id: string;
  sensor_name: string;
  min_temp: number;
  max_temp: number;
  avg_temp: number;
  min_temp_time: string;
  max_temp_time: string;
}

interface PDFReportProps {
  overallStats: TemperatureStats;
  sensorStats: SensorTemperatureStats[];
  startDate: Date;
  endDate: Date;
}

export const generatePDF = async ({
  overallStats,
  sensorStats,
  startDate,
  endDate
}: PDFReportProps): Promise<jsPDF> => {
  const doc = new jsPDF({
    orientation: 'portrait',
    unit: 'mm',
    format: 'a4',
    compress: true
  });
  
  // Title
  doc.setFontSize(20);
  doc.setTextColor(44, 62, 80); // Dark blue-gray
  doc.text('Temperature Report', 20, 20);
  
  // Date Range
  doc.setFontSize(12);
  doc.setTextColor(127, 140, 141); // Gray
  doc.text(`Report Period: ${format(startDate, 'EEEE do MMMM yyyy')} to ${format(endDate, 'EEEE do MMMM yyyy')}`, 20, 30);
  
  // Overall Stats
  doc.setFontSize(16);
  doc.setTextColor(44, 62, 80);
  doc.text('Overall Temperature Statistics', 20, 45);
  
  if (overallStats?.min_temp != null) {
    const overallData = [
      ['Metric', 'Value', 'Sensor', 'Time'],
      [
        'Minimum Temperature',
        `${overallStats.min_temp}°C`,
        overallStats.min_temp_sensor_name || 'Unknown',
        format(new Date(overallStats.min_temp_time), 'EEEE do MMMM yyyy h:mm a')
      ],
      [
        'Maximum Temperature',
        `${overallStats.max_temp}°C`,
        overallStats.max_temp_sensor_name || 'Unknown',
        format(new Date(overallStats.max_temp_time), 'EEEE do MMMM yyyy h:mm a')
      ],
      [
        'Average Temperature',
        `${overallStats.avg_temp}°C`,
        '',
        ''
      ]
    ];
    
    (doc as any).autoTable({
      startY: 50,
      head: [['Metric', 'Value', 'Sensor', 'Time']],
      body: overallData.slice(1),
      theme: 'grid',
      styles: { fontSize: 10 },
      headStyles: { fillColor: [66, 139, 202] }
    });
  } else {
    doc.setFontSize(12);
    doc.text('No temperature data available for the selected period', 20, 55);
  }
  
  // Sensor Stats
  doc.setFontSize(16);
  doc.text('Sensor Statistics', 20, doc.lastAutoTable?.finalY + 20 || 90);
  
  const sensorData = sensorStats.map(stat => [
    stat.sensor_name || 'Unknown',
    stat.min_temp ? `${stat.min_temp}°C` : 'N/A',
    stat.min_temp_time ? format(new Date(stat.min_temp_time), 'EEEE do MMMM yyyy h:mm a') : 'N/A',
    stat.max_temp ? `${stat.max_temp}°C` : 'N/A',
    stat.max_temp_time ? format(new Date(stat.max_temp_time), 'EEEE do MMMM yyyy h:mm a') : 'N/A',
    stat.avg_temp ? `${stat.avg_temp}°C` : 'N/A'
  ]);
  
  (doc as any).autoTable({
    startY: doc.lastAutoTable?.finalY + 25 || 95,
    head: [['Sensor', 'Min Temp', 'Min Time', 'Max Temp', 'Max Time', 'Avg Temp']],
    body: sensorData,
    theme: 'grid',
    styles: { fontSize: 9 },
    headStyles: { fillColor: [66, 139, 202] }
  });
  
  return doc;
};

const TemperatureReport = (props: PDFReportProps) => {
  const doc = generatePDF(props);
  doc.save(`temperature-report-${format(props.startDate, 'yyyy-MM-dd')}.pdf`);
  return null;
};

export default TemperatureReport;
